...
log-check-port-range-conflicts = Checking for occupied ports in range { $start_port }-{ $end_port }...
log-analyzing-network-ports = Analyzing listening network ports...
log-netstat-ss-command-error = Error executing `ss` or `netstat` command: { $error }. Utility might not be found or permissions are insufficient.
log-stage-pre-install-check-ports = Stage 0: Checking for port conflicts...
log-ss-command-error = Error executing `ss` command: { $error }. Utility might not be found or permissions are insufficient.
log-analyzing-network-ports = Analyzing listening network ports...
port-conflict-single-port-detail = Portmaster service port { $port } is occupied by processes: { $processes }
port-conflict-range-port-detail = Port { $port } (from range { $pm_range }) is occupied by processes: { $processes }
error-ports-occupied-by-other-processes = ❌ Critical Error: The following ports required by Portmaster are occupied by other processes on the host:
                                            { $conflicts }
